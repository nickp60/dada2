...
std::tuple<int,float*> get_best_genus(int *karray, float *out_logp, unsigned int arraylen, unsigned int n_kmers, unsigned int ngenus, float *lgk_probability) {
  unsigned int pos;
  float *lgk_v;
  int kmer, g, max_g = -1;
  ...
  float *logprob = (float *) calloc(ngenus, sizeof(float));;

  for(g=0;g<ngenus;g++) {
    lgk_v = &lgk_probability[g*n_kmers]; // P(w|g)
    logp = 0.0; // initialization
  ...
  *out_logp = max_logp;
  ...
  return std::make_tuple(max_g, logprob);
  // Rcpp::List output;
  // output["maxg"] = max_g;
...
struct AssignParallel : public RcppParallel::Worker
{
  ...
  AssignParallel(std::vector<std::string> seqs, std::vector<std::string> rcs, float *lgk_probability,
                 int *C_genusmat, double *C_unifs, int *C_rboot, int *C_rboot_tax, int *C_rval, float *C_logprob, float *C_logprobboot,
    ...
    : seqs(seqs), rcs(rcs), lgk_probability(lgk_probability),
      ...
      C_logprob(C_logprob),C_logprobboot(C_logprobboot), k(k), n_kmers(n_kmers), ngenus(ngenus), nlevel(nlevel), max_arraylen(max_arraylen), try_rc(try_rc) {}

  // Rprintf("Classify the sequences.\n");
  void operator()(std::size_t begin, std::size_t end) {
    size_t i, seqlen;
    unsigned int boot, booti, boot_match, arraylen, arraylen_rc;
    int max_g, max_g_rc, boot_g, g;
    int karray[9999];
    int karray_rc[9999];
    ...
    for(std::size_t j=begin;j<end;j++) {
      ...
      if(seqlen < 50) { // No assignment made for very short seqeunces
        ...
        // Find best hit
        tie(max_g, logprob) = get_best_genus(karray, &logp, arraylen, n_kmers, ngenus, lgk_probability);
        // max_g = output["maxg"];
        if(try_rc) { // see if rev-comp is a better match to refs
          ...
          tie(max_g_rc, logprob_rc) = get_best_genus(karray_rc, &logp_rc, arraylen_rc, n_kmers, ngenus, lgk_probability);
          // max_g_rc = output_rc["maxg"];
        ...
        for(g=0;g<ngenus;g++) {
          C_logprob[j*ngenus + g] = logprob[g];
        }
        ...
        for(boot=0;boot<NBOOT;boot++) {
          ...
          float bootmax = -9999;
          for(g=0;g<ngenus;g++) {
            if (boot_logprob[g] > bootmax ){
              bootmax = boot_logprob[g];
              C_logprobboot[j*NBOOT + boot] =  bootmax;
            }
          }
...
Rcpp::List C_assign_taxonomy2(std::vector<std::string> seqs, std::vector<std::string> rcs, std::vector<std::string> refs, std::vector<int> ref_to_genus, Rcpp::IntegerMatrix genusmat, bool try_rc, bool verbose) {
  ...
  for(i=0;i<nref;i++) {
    ...
    for(kmer=0;kmer<n_kmers;kmer++) {
      if(ref_kv[kmer]) {
        lgk_v[kmer]++;
        kmer_prior[kmer]++; // count number of seq observed for seq indexed by kmer
      }
    }
  }
  ...
  // Correct word priors
  for(kmer=0;kmer<n_kmers;kmer++) {
    kmer_prior[kmer] = (kmer_prior[kmer] + 0.5)/(1.0 + nref); // probability of word kmer found present
  }
  ...
  for(g=0;g<ngenus;g++) {
    ...
    // Rcpp::Rcout << g << ";" << *lgk_v << endl;

    for(kmer=0;kmer<n_kmers;kmer++) {
      lgk_v[kmer] = logf((lgk_v[kmer] + kmer_prior[kmer])/genus_num_plus1[g]); // p(w|g)
    }

    // Rcpp::Rcout << g << ";" << *lgk_v << endl;
  }

  // Rcpp::NumericMatrix rprob(ngenus,n_kmers);
  //
  // for(i=0;i<ngenus;i++) {
  //   for(j=0;j<n_kmers;j++) {
  //     rprob(i,j) = &lgk_probability[i*n_kmers + j];
  //   }
  // }
  ...
  float *C_logprobboot = (float *) calloc(nseq * NBOOT, sizeof(float));
  Rcpp::NumericMatrix rlogprob(nseq, ngenus);
  Rcpp::NumericMatrix rtoplogprob(nseq, NBOOT);

  AssignParallel assignParallel(seqs, rcs, lgk_probability, C_genusmat, C_unifs, C_rboot, C_rboot_tax, C_rval, C_logprob, C_logprobboot, k, n_kmers, ngenus, nlevel, max_arraylen, try_rc);
  ...
  for(i=0;i<nseq;i++) {
    for(j=0;j<ngenus;j++) {
      rlogprob(i,j) = C_logprob[i*ngenus + j];
    }
  }
  for(i=0;i<nseq;i++) {
    for(j=0;j<NBOOT;j++) {
      rtoplogprob(i,j) = C_logprobboot[i*NBOOT + j];
    }
  }
  ...
  return(Rcpp::List::create(_["tax"]=rval, _["boot"]=rboot, _["boot_tax"]=rboot_tax, _["prob"]=rlogprob, _["boot_top_prob"]=rtoplogprob));
}
